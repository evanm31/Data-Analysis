...
```
To get a better sense of its accuracy: 
...
```
78.6% - not bad. Let's graph the ROC curve to get a sense of the true positive rate  vs false positive rate and calculate the area under the curve (AUC). The closer to one the AUC is, the more accurate the regression model is. 
```{r}
library(ROCR)
