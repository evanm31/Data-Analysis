...
## Predictive Analysis
Let's try using logistic regression to predict if an employee from the general dataset will quit or not.
```{r}
employees <- mutate(employees, quit = is.na(employees$quit_date))
employees$quit <- employees$quit * 1
employees_train <- employees[1:16933,]
employees_test <- employees[16933:24189,]
model <- glm(quit ~ join_date + seniority + salary + dept ,family=binomial(link='logit'),data=employees_train)
summary(model)
anova(model, test="Chisq")
```
The high deviance in the `join_date` variable compared to the rest of the variables in the ANOVA table indicates that the model is almost solely going off of that variable, and that perhaps another model would be better fit to this data than logistic regression, For example, it would be interesting to try prediciting the number of days one who quit worked based on the rest of the variables, and then apply to model to more general employees. Regardless, let's try going further with this (flawed) logistic regression model and get a better sense of its accuracy on the test data: 

```{r}
fitted.results <- predict(model,newdata=subset(employees_test,select=c(1,2,3,4)),type='response')
fitted.results <- ifelse(fitted.results > 0.5,1,0)
misClasificError <- mean(fitted.results != employees_test$quit)
print(paste('Accuracy',1-misClasificError))
```
Around 78%. Let's now graph the ROC curve to get a sense of the true positive rate  vs. false positive rate and calculate the area under the curve (AUC). The closer to one the AUC is, the more accurate the regression model is. 
```{r}
library(ROCR)
p <- predict(model, newdata=subset(employees_test,select=c(1, 2,3,4)), type="response")
pr <- prediction(p, employees_test$quit)
